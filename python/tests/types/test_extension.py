...
from rikai.types.pandas import Box2dDtype, ImageDtype


def test_image(tmp_path):
    # Convert string to Image
    s = pd.Series(
        np.array(["s3://bucket/path/to/image.jpg"]), dtype=ImageDtype()
    )
...
def test_nested(tmp_path):
    ...
    s2 = pd.Series(
        [
            [
                {"label": "foo", "box": Box2d(0, 0, 100, 100), "score": 0.8},
                {"label": "bar", "box": Box2d(0, 0, 100, 200), "score": 0.7},
            ]
        ]
    )

    df = pd.DataFrame({"image": s1, "annotations": s2})
