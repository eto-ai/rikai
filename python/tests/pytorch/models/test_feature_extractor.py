...
def test_resnet_features(asset_path: Path, spark: SparkSession, tmp_path: Path):
    ...
    USING '{uri}'
    """)
    df = spark.sql(f"SELECT ML_PREDICT(resnet_features, image('{asset_path / 'cat.jpg'}')) as embedding")

    assert df.schema == StructType([StructField("embedding", ArrayType(FloatType()))])
